...
describe('AuthService', () => {
  ...
  describe('when the setLoginCookies method is called', () => {
    it('should set the verified cookie', () => {
      ...
      expect(res.cookie).toHaveBeenCalledWith('verified', user.verified, {
        httpOnly: false,
      });
    });
    ...
    it('should set reset cookie and set within redis', () => {
      ...
      expect(res.cookie).toHaveBeenCalledWith(
        'refresh_token',
        {},
        { httpOnly: true, expires: expect.any(Date) }
      );
    ...
    it('should set the authenticated cookie', () => {
      ...
      expect(res.cookie).toHaveBeenCalledWith('authenticated', true, {
        httpOnly: false,
    ...
    it('should delete key from redis when email exists', async () => {
      const req = {
        cookies: {
          ...
          // eslint-disable-next-line camelcase
          access_token:
            'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwiZW1haWwiOiJhQGEuY29tIiwiaWF0IjoxNTE2MjM5MDIyfQ.mmqUsu7kpT7M9QUYj69X1TNVCyatAPgky9JXtrSuHrU',
        },
      } as Request;
