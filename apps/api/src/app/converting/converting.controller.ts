...
export class ConvertingController {
  ...
  async exportSetToQuizletTxt(
    @Param() params: QuizletExportParams,
    ...
    const txt = this.convertingService.convertSetToQuizletTxt(
      set,
      params.sideDiscriminator,
      params.cardDiscriminator
    );
    if (!txt)
      throw new BadRequestException(
        "At least one card in the set contains side or card discriminator characters. The set must not contain the characters being used to format the exported set."
      );
  ...
  async exportSetToAnkiApkg(
    ...
    if (!apkg)
      throw new InternalServerErrorException(
        "Error converting set to apkg file"
      );
  ...
  async exportSetToCsv(
    ...
    if (!csv)
      throw new InternalServerErrorException(
        "Error converting set to csv file"
      );
  ...
  @Post("import/quizlet")
  async importSetFromQuizletTxt(
    @Body(HtmlDecodePipe) body: ImportSetFromQuizletDto,
    @Request() req: ExpressRequest
    ...
    const cards = this.convertingService.quizletStringToCards(
      body.set,
      body.sideDiscriminator,
      body.cardDiscriminator
    );
    if (!cards)
      throw new BadRequestException("The set is not correctly formatted");
  ...
  @Post("import/apkg")
  async importSetFromAnkiApkg(
    @Body(HtmlDecodePipe) body: ImportSetFromFileDto,
    @Request() req: ExpressRequest,
    ...
    const decoded = await this.convertingService.apkgToCardsAndMedia(
      file.buffer,
      uuid
    );
    if (!decoded)
      throw new UnsupportedMediaTypeException({
        status: "fail",
        message: "Set is incompatible to import",
      });
    ...
    for (const media of decoded.media) {
      const card = create.cards.find(
        (c) => c.term.includes(media) || c.definition.includes(media)
      );
      if (!card) continue;
